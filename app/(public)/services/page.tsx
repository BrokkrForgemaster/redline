...
const fallbackServices: ServiceCard[] = [
  {
    title: "Lawn Mowing",
    slug: "lawn-mowing",
    description: "Keep your lawn looking sharp with our weekly and bi-weekly mowing services. We handle edging, trimming, and cleanup so you don't have to.",
    features: ["Weekly & bi-weekly scheduling", "Edging & string trimming", "Grass clipping cleanup", "Consistent cut height"],
  ...
  },
  {
    title: "Landscaping",
    slug: "landscaping",
    description: "From garden beds to complete outdoor transformations, our landscaping services bring your vision to life with professional design and installation.",
    features: ["Custom landscape design", "Mulching & garden beds", "Shrub & tree planting", "Hardscaping & retaining walls"],
  ...
  },
  {
    ...
    description: "Revive your lawn with professional core aeration and overseeding. We relieve soil compaction and fill in thin spots for a thicker, healthier turf.",
    features: ["Core aeration", "Premium seed blend", "Soil compaction relief", "Thicker, healthier turf"],
  ...
  },
  {
    title: "Snow Removal",
    slug: "snow-removal",
    description: "Don't let winter slow you down. Our snow plowing and ice management keeps your driveways, parking lots, and walkways safe and clear.",
    features: ["Residential & commercial plowing", "Sidewalk & walkway clearing", "Ice treatment & salting", "24/7 storm response"],
...
export default async function ServicesPage() {
  ...
  if (wpPages.length > 0) {
    services = fallbackServices.map((fallback) => {
      const wpPage = wpPages.find((p) => wpSlugToRouteSlug[p.slug] === fallback.slug);
    ...
    <>
      <section className="bg-offwhite py-16 px-6">
        <div className="max-w-7xl mx-auto text-center animate-fade-in-up">
          <span className="text-sm font-semibold uppercase tracking-widest text-lawn">Year-Round Care</span>
          <h1 className="mt-2 text-4xl md:text-5xl font-bold text-charcoal">Our Services</h1>
          <p className="mt-4 text-lg text-muted max-w-2xl mx-auto leading-relaxed">
            Year-round property maintenance for homes and businesses across Central Kentucky.
          </p>
        </div>
      </section>
      ...
      <section className="bg-white py-24 px-6">
        <div className="max-w-7xl mx-auto space-y-24">
          {services.map((service, i) => (
            <div key={service.slug} className={`flex flex-col md:flex-row items-center gap-12 ${i % 2 === 1 ? "md:flex-row-reverse" : ""}`}>
              ...
              <div className="w-full md:w-1/2 animate-fade-in-up delay-200">
                ...
                <div className="mt-8 flex flex-wrap gap-4">
                  <Link href={`/services/${service.slug}`} className={`${service.buttonClass} text-white text-sm font-semibold uppercase tracking-wide px-6 py-3 rounded-md transition-all shadow-sm hover:shadow-md hover:-translate-y-0.5`}>
                    Learn More
                  </Link>
                  <Link href="/contact" className="border border-border text-charcoal text-sm font-semibold uppercase tracking-wide px-6 py-3 rounded-md hover:bg-offwhite transition-colors">
                    Get a Quote
                  </Link>
                </div>
              </div>
      ...
      <section className="bg-offwhite py-16 px-6">
        <div className="max-w-7xl mx-auto text-center">
          ...
          <Link href="/contact" className="inline-block mt-8 bg-redline text-white text-sm font-semibold uppercase tracking-wide px-8 py-3.5 rounded-md hover:bg-redline-dark transition-all shadow-lg hover:shadow-xl hover:-translate-y-0.5">
            Request a Quote
          </Link>
        </div>
      </section>
